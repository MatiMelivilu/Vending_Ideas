...
class SerialGUI(tk.Tk):
    ...
    def __init__(self, log_file, *args, **kwargs):
        ...
        if (self.dispositivo_conectado('/dev/ttyACM0')):
                POS_CONNECT = 1
    ...
    def control_message(self, mensaje):
        ...
        if ((ID_hex == "1300" or ID_hex == "1306") and largo == 7):
            TIMEOUT = 1
            ...
            elif pago == 'El POS no está conectado':
                pass
            ...
            if ('130417' in mensaje):
                ...
                STATE = 0
                TIMEOUT=0
                if self.pos_port.is_open:
                    POS_CONNECT = 1
                else:
                    print('reconectando POS')
                    self.pos_port.open()
                    POS_CONNECT = 1
            else:
                pass
    ...
    def read_serial(self):
        ...
        while True:
            ...
            if (self.dispositivo_conectado('/dev/ttyACM0') and POS_CONNECT == 1):
                if self.pos_port.is_open:
                    if self.pos_port.in_waiting > 0:
                        mensaje = self.pos_port.read(self.pos_port.in_waiting)
                        print(f"Mensaje recibido: {mensaje.decode('utf-8')}")
                        print(f"Mensaje recibido: {mensaje}")
                        
                        if mensaje == b'\x020911\x03\x08':
                            print('el mensaje especifico es:', mensaje.decode('utf-8'))
                            self.send_message4()
                            self.pos_port.close()
                    ...
                    print('el puerto ya esta cerrado')
